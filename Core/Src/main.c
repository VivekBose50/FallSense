...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	char msg1[] = "the LEDs are blinkin...\n";
  ...
  if (BSP_COM_Init(COM1, &BspCOMInit) != BSP_ERROR_NONE)
  {
	...
	HAL_I2C_Mem_Read(&hi2c1, IMU_ADDR, CTRL1_XL,
	                 I2C_MEMADD_SIZE_8BIT,
	                 &check, 1, HAL_MAX_DELAY);

	char msg2[50];
	sprintf(msg2, "CTRL1_XL: 0x%X\r\n", check);
	HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)msg2, strlen(msg2), HAL_MAX_DELAY);
	if (HAL_I2C_IsDeviceReady(&hi2c1, IMU_ADDR, 3, 100) == HAL_OK)
	{
		HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)msgSuccess, sizeof(msgSuccess) - 1, HAL_MAX_DELAY);
	}
	else
	{
		HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)msgFailure, sizeof(msgFailure) - 1, HAL_MAX_DELAY);
	}
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  uint8_t raw[6];
	     ...
	     HAL_I2C_Mem_Read(&hi2c1, IMU_ADDR, 0x28, I2C_MEMADD_SIZE_8BIT, raw, 6, HAL_MAX_DELAY);
	  ...
	  if(BSP_PB_GetState(BUTTON_USER) == BUTTON_PRESSED)
	  {
		  ...
		  HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)msg1, sizeof(msg1) - 1, HAL_MAX_DELAY);
